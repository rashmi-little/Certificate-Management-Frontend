...
export const fetchRequestLog = (filters, hasMoreRef) => async (dispatch) => {
  ...
  try {
    const { data } = await api.get(`${CERTIFICATESERVICE_COMMON_URL}`, {
      params,
    });
    dispatch({
...
export const fetchRequestView = (requestId) => async (dispatch) => {
  ...
  try {
    const { data } = await api.get(`${CERTIFICATESERVICE_COMMON_URL}/view`, {
      params: { requestId },
    });
  ...
  (payload, onSuccess) => async (dispatch) => {
    ...
    try {
      const { data } = await api.put(
        `${CERTIFICATESERVICE_COMMON_URL}/certificate-request`,
        payload
      );
